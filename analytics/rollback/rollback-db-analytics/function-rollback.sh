...
function rollback-analytics-db
	{
					...
					if [ -e /tmp/${DATABASE}_${BACKUP_NAME}.dump ]
						then
							cd /tmp
							dropdb ${DATABASE} && createdb -O dealtap ${DATABASE}
							pg_restore -l /tmp/${DATABASE}_${BACKUP_NAME}.dump >/tmp/analytics.list
							...
							pg_restore -d ${DATABASE} -L /tmp/analytics.list /tmp/${DATABASE}_${BACKUP_NAME}.dump &>> /var/log/deploy/$ENVIRONMENT/err_rollback_analytics_db_${BACKUP_NAME}.log
							if [ $? -eq 0 ]
							then
								echo "rollback analytics db success"
								rm -f /tmp/analytics.list 
								rm -f /tmp/${DATABASE}_${BACKUP_NAME}.dump
							else 
					...
					echo "================rollback analytics db end $(date +%T)=========="
					echo "================rollback analytics db end $(date +%T)==========" >> /var/log/deploy/$ENVIRONMENT/err_rollback_analytics_db_${BACKUP_NAME}.log
	}