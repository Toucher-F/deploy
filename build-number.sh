#!/bin/bash
read -t 30 -p "Please input your action [test/formal]?" action
...
##########################################################################################
read -p "Whether to create build number for application or not[y/n]?" is_deploy_app
...
##########################################################################################
read -p "Whether to create build number for analytics or not[y/n]?" is_deploy_ana
...
if [ "${is_deploy_ana}x" == "yx" ];then
	read -t 30 -p "Please input the analytics branch [as:staging]?" ana_branch
