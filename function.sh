		...
		if [ -d ${DTPATH}/app-ui -a -d ${DTPATH}/app-core ]
			then 
				...
				if [ ! -e ${DTPATH}/backup/${ENVIRONMENT}/${BACKUP_NAME}/app-core.tar.gz ]
					then 
						cd ${DTPATH}
						cp -r app-core ${DTPATH}/backup/${ENVIRONMENT}/${BACKUP_NAME}/
						cd ${DTPATH}/backup/${ENVIRONMENT}/${BACKUP_NAME}/
						tar -zcf ./app-core.tar.gz ./app-core >> ${LOGDIR}err_php_${BACKUP_NAME}.log 2>&1
						rm -rf ./app-core
				else
					echo "Project has been backup without backup again"
					return 10
...
if [ -e ${DTPATH}/app-core/app/config/config.php ];then
	...
	sed -i '/Enums\\Config::URL_ASSETS_EMAIL/c\\tEnums\\Config::URL_ASSETS_EMAIL => '\'${Config_URL_ASSETS_EMAIL}\'',' ./config.php
else
	return 1
fi
...
function roles
		{
			...
			if
			curl -s -k -A Chrome/55.0.2883.87 -D /tmp/roles-http -o /tmp/roles-conment "${ROLESLINK}?pk=${PUBLIC_KEY}&qt=$(date +%s)" &>> ${LOGDIR}err_php_${BACKUP_NAME}.log
			then
				sleep 5
...
function forms
		{
			...
			PUBLIC_KEY=$(grep 'Enums\\ConfigAuth::PUBLIC_KEY' ${DTPATH}/app-core/app/config/config.php | cut -d "'" -f 2)
			if
			curl -s -k -A Chrome/55.0.2883.87 -D /tmp/forms-http -o /tmp/forms-conment "${FORMSLINK}?pk=${PUBLIC_KEY}&qt=$(date +%s)" &>> ${LOGDIR}err_php_${BACKUP_NAME}.log 
			then
				sleep 5
...
function invalidate
		{
			echo "=====================invalidate Start $(date +%T)====================" >> ${LOGDIR}err_php_${BACKUP_NAME}.log
			PUBLIC_KEY=$(grep 'Enums\\ConfigAuth::PUBLIC_KEY' ${DTPATH}/app-core/app/config/config.php | cut -d "'" -f 2)
			if
			curl -s -k -A Chrome/55.0.2883.87 -D /tmp/invalidate-http -o /tmp/invalidate-conment "${Invalidate}?pk=${PUBLIC_KEY}&qt=$(date +%s)" &>> ${LOGDIR}err_php_${BACKUP_NAME}.log 
			then
				sleep 5
				formscode=$(grep "HTTP" /tmp/invalidate-http | awk '{printf $2}')
				if [ $formscode -ne 200 ]
					then 
						rm -f /tmp/invalidate-http /tmp/invalidate-conment
						return 1
				else 
					rm -f /tmp/invalidate-http
					if 
						grep -niw "error" /tmp/invalidate-conment
					then
						rm -f /tmp/invalidate-conment
						echo "invalidate document output a error"
						return 1
					else
						echo "=====================invalidate end $(date +%T)====================" >> ${LOGDIR}err_php_${BACKUP_NAME}.log
						return 0
					fi
				fi
			else
