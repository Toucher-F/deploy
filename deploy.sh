#/bin/bash
read -p "Please input the project environment[30QA/30staging/30prod/test30QA/test30staging/test30prod]?" ENVIRONMENT
...
##########################################################################################
read -p "Whether to deploy application project or not[y/n]?" is_deploy_app
...
if [ "${is_deploy_app}x" == "yx" ];then
	...
	read -p "Please input the application build number [as:30-172024-0]?" app_build_number
...
################################################################################################
read -p "Whether to deploy analytics project or not[y/n]?" is_deploy_ana
...
if [ "${is_deploy_ana}x" == "yx" ];then
	...
	read -p "Please input the analytics build number [as:30-172024-0]?" ana_build_number
